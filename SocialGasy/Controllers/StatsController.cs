    ...
    public class StatsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? district = null)
        {
            ...
            var finalStats = results.Select(c => {
                var dob = c.Contains("DateOfBirth") && c["DateOfBirth"].IsBsonDateTime ? c["DateOfBirth"].ToUniversalTime() : DateTime.UtcNow;
                int age = DateTime.UtcNow.Year - dob.Year;
                string group = age < 18 ? "Mineur (<18)" : age <= 35 ? "Jeune (18-35)" : age <= 60 ? "Adulte (36-60)" : "Senior (>60)";
                return new { 
                    gender = c.Contains("Gender") ? c["Gender"].AsString : "Unknown", 
                    ageGroup = group 
                };
            })
            ...
            ViewBag.Stats = JsonSerializer.Serialize(finalStats, options);
            return View();
        }
    }
}